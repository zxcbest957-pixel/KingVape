...
local function loadAnalytics()
	task.spawn(function()
		pcall(function()
			...
			-- Only read from cache if ForceUpdate is not active
			if isfile(path) and not shared.ForceUpdate then
				pcall(function() content = readfile(path) end)
			end
			
			-- If not cached, or cached file has placeholder or is invalid, download fresh
			...
			if content and content ~= '' and not content:find('404: Not Found') then
				...
				if fn then
					...
					if not success and execErr then
						-- print only in studio/debug
					end
				end
			end
		end)
